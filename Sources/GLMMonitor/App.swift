...
struct GLMMonitorApp: App {
    ...
    init() {
        ...
        vm.checkForUpdates = { [updaterController] in
    ...
    var body: some Scene {
        MenuBarExtra {
            ...
            if viewModel.statusBarMode == .pie, let pct = sessionPercentage {
                Image(nsImage: combinedPieImage(percentage: pct, isDark: viewModel.isDarkMode))
            } else {
                HStack(spacing: 4) {
                    Image(nsImage: loadAppIcon(isDark: viewModel.isDarkMode))
    ...
    private func combinedPieImage(percentage: Int, isDark: Bool) -> NSImage {
        ...
        ) else { return loadAppIcon(isDark: isDark) }

        // Draw app icon on the left
        let icon = loadAppIcon(isDark: isDark)
        ...
        // Background circle - lighter in dark mode
        let bgAlpha: CGFloat = isDark ? 0.5 : 0.3
        cgCtx.setFillColor(CGColor(red: 1, green: 1, blue: 1, alpha: bgAlpha))
        cgCtx.fillEllipse(in: pieRect)

        // Filled arc with dark-mode-aware colors
        ...
        if fraction > 0 {
            ...
            if percentage > 90 {
                ...
                color = isDark
                    ? CGColor(red: 1, green: 0.7, blue: 0.3, alpha: 1)
                    : CGColor(red: 1, green: 0.6, blue: 0.2, alpha: 1)
            } else {
                color = isDark
                    ? CGColor(red: 0.3, green: 0.9, blue: 0.4, alpha: 1)
        ...
        // Outline - white in dark mode, black in light mode
        let outlineColor = isDark
            ? CGColor(red: 1, green: 1, blue: 1, alpha: 1)
            : CGColor(red: 0, green: 0, blue: 0, alpha: 1)
        cgCtx.setStrokeColor(outlineColor)
        cgCtx.setLineWidth(1.5)
        cgCtx.strokeEllipse(in: pieRect)

        guard let cgImage = cgCtx.makeImage() else { return loadAppIcon(isDark: isDark) }
